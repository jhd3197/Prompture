...
def process_contacts_batch(texts: list[str], model_name: str = "lmstudio/deepseek/deepseek-r1-0528-qwen3-8b"):
    ...
    for i, text in enumerate(texts):
        ...
        try:
            contact = extract_with_model(ContactInfo, text, model_name)
            ...
            results.append({"index": i, "original_text": text, "extracted_data": contact, "success": True})

        except Exception as e:
            errors.append(f"Extraction failed for text {i}: {e!s}")
            results.append(
                {"index": i, "original_text": text, "extracted_data": None, "success": False, "error": str(e)}
            )
