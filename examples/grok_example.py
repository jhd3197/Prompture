"""
Example: Using manual_extract_and_jsonify with Grok.

This script demonstrates:
1. Initializing the Grok driver manually (ignoring AI_PROVIDER).
2. Extracting structured information from text using a JSON schema.
3. Overriding the Grok model per call with `model_name`.
...
# === FIRST EXAMPLE: Default instruction with Grok 4 Fast Reasoning ===
print("Extracting information into JSON with default instruction...")

result = extract_and_jsonify(
    text=text,
    json_schema=json_schema,
    model_name="grok/grok-4-fast-reasoning"  # explicitly select model
)
...
# === SECOND EXAMPLE: Custom instruction with Grok 3 Mini ===
print("\n\n=== SECOND EXAMPLE - CUSTOM INSTRUCTION & DIFFERENT MODEL ===")
print("Extracting information with custom instruction using Grok 3 Mini...")

custom_result = extract_and_jsonify(
    text=text,
    json_schema=json_schema,
    instruction_template="Parse the biographical details from this text:",
    model_name="grok/grok-3-mini"  # override model here
)
