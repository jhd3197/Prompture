...
from prompture import extract_and_jsonify, stepwise_extract_with_model
...
def test_stepwise_method(model_name=None):
    ...
    try:
        ...
        if model_instance is None:
            raise ValueError(
                f"Model validation failed: {result.get('usage', {}).get('validation_errors', 'Unknown error')}"
            )
        ...
        print(f"Stepwise extraction error: {e!s}")  # Add detailed error logging
        return {
            "success": False,
            "method": "stepwise_extract_with_model",
            "model_name": model_name or "default",
...
def test_manual_method(model_name=None):
    ...
    try:
        result = extract_and_jsonify(
            text=SAMPLE_TEXT, json_schema=PERSON_SCHEMA, model_name=model_name if model_name else None
        )
...
def get_stepwise_advantage(result):
    ...
    is_valid, _errors = validate_extraction_accuracy(result)
    if is_valid:
...
def validate_extraction_accuracy(result):
    ...
    if not person.profession or not any(
        role in person.profession.lower() for role in ["manager", "engineering", "software"]
    ):
...
def print_comparison_table(results):
    ...
    headers = ["Method", "Model", "Success", "Total Tokens", "Calls", "Validated", "Cost ($)", "Why Stepwise Wins"]
    ...
    if stepwise_results and manual_results:
        ...
        print("3. Average token usage:")
        print(
            f"   - Stepwise: {avg_stepwise_tokens:.0f} tokens ({avg_stepwise_tokens / len(stepwise_results):.0f} per model)"
        )
        ...
        print("5. Validation Success Rate:")
        print(
            f"   - Stepwise: {validated_stepwise}/{len(stepwise_results)} models ({validated_stepwise / len(stepwise_results) * 100:.1f}%)"
        )
        print(
            f"   - Manual: {validated_manual}/{len(manual_results)} models ({validated_manual / len(manual_results) * 100:.1f}%)"
        )
