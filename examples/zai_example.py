"""
Example: Using extract_and_jsonify with Z.ai (Zhipu AI).

This script demonstrates:
1. Initializing the Z.ai driver manually (ignoring AI_PROVIDER).
2. Extracting structured information from text using a JSON schema.
3. Overriding the Z.ai model per call with `model_name`.
...
# === FIRST EXAMPLE: Default instruction with GLM-4.7 ===
print("Extracting information into JSON with default instruction...")

result = extract_and_jsonify(
    text=text,
    json_schema=json_schema,
    model_name="zai/glm-4.7",  # explicitly select model
)
...
# === SECOND EXAMPLE: Custom instruction with GLM-4.7 ===
