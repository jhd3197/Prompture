...
def compare_formats(
    ...
    stats = {
        "data_info": {
            ...
            "uniform_structure": all(
                set(item.keys()) == set(array_data[0].keys()) 
                for item in array_data
            ) if array_data else False
        },
        "json_formatted": {
            "text": json_text,
    ...
    if toon_text:
        ...
        token_savings_pct_compact = (token_savings_compact / json_compact_tokens * 100) if json_compact_tokens > 0 else 0
...
def print_comparison_report(stats: Dict[str, Any]) -> None:
    ...
    print(f"{'JSON (formatted)':<15} {json_fmt['character_count']:>12} {json_fmt['estimated_tokens']:>12} {'-':>15} {'-':>12}")
    ...
    print(f"{'JSON (compact)':<15} {json_cmp['character_count']:>12} {json_cmp['estimated_tokens']:>12} {compact_savings_chars:>15} {compact_savings_pct:>11.1f}%")
    
    # TOON
    if "toon" in stats:
        toon_data = stats["toon"]
        savings_fmt = stats["savings_vs_formatted"]
        print(f"{'TOON':<15} {toon_data['character_count']:>12} {toon_data['estimated_tokens']:>12} {savings_fmt['character_savings']:>15} {savings_fmt['character_savings_percent']:>11.1f}%")
    else:
    ...
    if "savings_vs_formatted" in stats:
        ...
        if savings['token_savings_percent'] > 50:
            print(f"   🎉 Excellent savings! TOON is highly effective for this data.")
        elif savings['token_savings_percent'] > 25:
            print(f"   ✅ Good savings! TOON provides meaningful token reduction.")
        elif savings['token_savings_percent'] > 0:
            print(f"   📈 Modest savings. TOON provides some benefit.")
        else:
            print(f"   ⚠️  No savings. This data may not benefit from TOON format.")
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--interactive":
        ...
        if not TOON_AVAILABLE:
            ...
            print(f"   pip install python-toon")
