"""
Example: Using manual_extract_and_jsonify with Azure OpenAI.
...
# 1. Instantiate the Azure driver explicitly
azure_driver = get_driver("azure")

# 2. Define the raw text to parse
text = "Maria is 32 years old and works as a software developer in New York. She loves hiking and photography."

# 3. Define the JSON schema for expected output
json_schema = {
    "type": "object",
    "properties": {
        "name": {"type": "string"},
...
result = manual_extract_and_jsonify(
    ...
    model_name="gpt-4o-mini"  # explicitly override to a known Azure deployment
)
...
custom_result = manual_extract_and_jsonify(
    ...
    model_name="gpt-4"  # override to a different Azure deployment/model
)
