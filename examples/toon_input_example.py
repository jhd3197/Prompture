...
ANALYSIS_SCHEMA = {
    ...
    "properties": {
        ...
        "categories": {"type": "array", "items": {"type": "string"}, "description": "List of unique categories"},
        "premium_products": {"type": "array", "items": {"type": "string"}, "description": "Names of products over $20"},
    },
...
def demonstrate_extract_from_data():
    ...
    try:
        result = extract_from_data(
            data=PRODUCT_DATA, question=question, json_schema=ANALYSIS_SCHEMA, model_name=MODEL_TO_TEST
        )
...
def demonstrate_extract_from_pandas():
    ...
    try:
        result = extract_from_pandas(df=df, question=question, json_schema=pricing_schema, model_name=MODEL_TO_TEST)
...
def demonstrate_data_with_key():
    ...
    nested_data = {"products": PRODUCT_DATA, "total_count": len(PRODUCT_DATA), "page": 1, "per_page": 50}
    ...
    print("Data structure: API response with 'products' key")
    print(f"Question: {question}")
    ...
    try:
        ...
        for i, product in enumerate(products, 1):
            print(
                f"  {i}. {product.get('name', 'N/A')} - ${product.get('price', 0):.2f} ({product.get('supplier', 'N/A')})"
            )
...
def main():
    ...
    if successful_tests > 0:
        ...
        print("   • Significant token reduction (typically 45-60% for uniform arrays)")
        print("   • Automatic conversion of JSON arrays and DataFrames")
        print("   • Token usage tracking and savings analysis")
        print("   • Flexible question-answering over structured data")
    else:
