...
def get_decision_driver_for_model(model_str: str) -> DecisionDriver:
    ...
    return cast(DecisionDriver, factory(model_id))


def get_async_decision_driver_for_model(model_str: str) -> AsyncDecisionDriver:
    """Instantiate an async decision driver from a ``"provider/model"`` string."""
    parts = model_str.split("/", 1)
    provider = parts[0].lower()
    model_id = parts[1] if len(parts) > 1 else None
    factory = get_async_decision_driver_factory(provider)
    return cast(AsyncDecisionDriver, factory(model_id))
