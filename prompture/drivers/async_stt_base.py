"""Async base class for speech-to-text (STT) drivers."""
...
logger = logging.getLogger("prompture.async_stt_driver")


class AsyncSTTDriver:
    """Async adapter base for speech-to-text. Implement ``async transcribe(audio, options)``.

    Response contract is identical to :class:`STTDriver`.
    """
    ...
    async def transcribe_with_hooks(self, audio: bytes, options: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            resp = await self.transcribe(audio, options)
        ...
        meta = resp.get("meta", {})
        logger.debug(
            "[stt] async transcribe driver=%s duration=%.1fs cost=%.6f elapsed=%.0fms",
            driver_name,
