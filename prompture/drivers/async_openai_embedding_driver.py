"""Async OpenAI text embedding driver. Requires the ``openai`` package (>=1.0.0)."""
...
class AsyncOpenAIEmbeddingDriver(EmbeddingCostMixin, AsyncEmbeddingDriver):
    """Async text embedding via OpenAI Embeddings API."""
    ...
    async def embed(self, texts: list[str], options: dict[str, Any]) -> dict[str, Any]:
        """Generate embeddings using OpenAI Embeddings API (async).
