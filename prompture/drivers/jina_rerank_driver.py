"""Jina AI rerank driver.

Uses Jina's ``/v1/rerank`` endpoint.  Requires ``JINA_API_KEY``.
"""
...
class JinaRerankDriver(RerankDriver):
    """Jina AI rerank driver.
    ...
    def rerank(
        ...
        **options: Any,
        ...
        try:
            ...
            error_msg = f"Jina rerank API request failed: {e!s}"
            if body:
                error_msg += f"\nResponse: {body}"
            raise RuntimeError(error_msg) from e
        except requests.exceptions.RequestException as e:
            raise RuntimeError(f"Jina rerank API request failed: {e!s}") from e
