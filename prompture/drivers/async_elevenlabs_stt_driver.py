"""Async ElevenLabs STT driver. Uses httpx for the REST API."""
...
class AsyncElevenLabsSTTDriver(AudioCostMixin, AsyncSTTDriver):
    """Async speech-to-text via ElevenLabs REST API."""

    supports_timestamps = False
    supports_language_detection = True

    AUDIO_PRICING = ElevenLabsSTTDriver.AUDIO_PRICING
    ...
    async def transcribe(self, audio: bytes, options: dict[str, Any]) -> dict[str, Any]:
        """Transcribe audio using ElevenLabs Speech-to-Text API (async).
        ...
        async with httpx.AsyncClient() as client:
            resp = await client.post(url, headers=headers, files=files, data=data, timeout=120)
