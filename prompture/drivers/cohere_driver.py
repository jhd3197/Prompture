"""Cohere LLM driver implementation.

Uses Cohere's v2 Chat API at ``https://api.cohere.com/v2/chat``.  Cohere's
chat API is *not* OpenAI-compatible — request and response shapes differ.

Requires the ``COHERE_API_KEY`` env var (or pass ``api_key=`` explicitly).
"""
...
class CohereDriver(CostMixin, Driver):
    """Cohere v2 chat driver.
    ...
    def _extract_text(message: dict[str, Any]) -> str:
        """Pull text from Cohere v2's array-of-content response shape."""
    ...
    def _do_generate(self, messages: list[dict[str, Any]], options: dict[str, Any]) -> dict[str, Any]:
        ...
        # Native JSON output: Cohere v2 supports ``response_format``.
