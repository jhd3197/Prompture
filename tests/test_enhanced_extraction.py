...
class TestStepwiseExtractionBasic:
    ...
    def test_input_validation(self):
        ...
        with pytest.raises(ValueError, match="Text input cannot be empty"):
            stepwise_extract_with_model(
                PersonModel, "", model_name="test/model"
            )
        
        # None text should raise error
        with pytest.raises(ValueError, match="Text input cannot be empty"):
            stepwise_extract_with_model(
                PersonModel, None, model_name="test/model"
            )
    ...
    def test_field_validation(self):
        ...
        result = None  # We'll mock this in integration tests
    ...
    def test_successful_extraction_all_fields(self, mock_extract):
        ...
        result = stepwise_extract_with_model(
            PersonModel, 
            "John Doe is a 30-year-old engineer. Email: john@example.com",
            model_name="test/model"
        )
        ...
        for field_name in ["name", "age", "email", "occupation"]:
            ...
            assert field_results[field_name]["used_default"] == False
...
class TestStepwiseExtractionWithFailures:
    ...
    def test_extraction_failure_with_defaults(self, mock_extract):
        ...
        result = stepwise_extract_with_model(
            PersonModel,
            "John Doe is an engineer. Email: john@example.com",
            model_name="test/model"
        )
        ...
        assert field_results["age"]["used_default"] == True
...
class TestStepwiseExtractionWithFieldDefinitions:
    ...
    def test_extraction_with_field_definitions(self, mock_extract):
        ...
        assert field_results["value"]["used_default"] == True
    ...
    def test_custom_field_definitions(self, mock_extract):
        ...
        mock_extract.side_effect = [
            Exception("Extraction failed for name"),
            Exception("Extraction failed for value")
        ]
        
        result = stepwise_extract_with_model(
            SimpleModel,
            "Some text without clear info",
            model_name="test/model",
            field_definitions=custom_definitions
        )
        ...
        if "model" in result:
            ...
            # Check field results
            field_results = result["field_results"]
            assert field_results["name"]["used_default"] == True
            assert field_results["value"]["used_default"] == True
        else:
