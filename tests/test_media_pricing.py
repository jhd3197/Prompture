"""Tests for media pricing + the estimate_cost quote (G16)."""
...
class TestEstimateCost:
    def test_per_image_times_n(self):
        assert estimate_cost("muapi/nano-banana", n=3) == round(0.039 * 3, 6)

    def test_per_second_times_duration(self):
        assert estimate_cost("muapi/kling-video-v2-1", duration_seconds=5) == round(0.10 * 5, 6)

    def test_per_second_via_duration_alias(self):
        assert estimate_cost("runway/veo3", duration=2) == round(0.40 * 2, 6)

    def test_per_character(self):
        assert estimate_cost("elevenlabs/eleven_multilingual_v2", characters=1000) == round(0.00003 * 1000, 6)

    def test_per_job(self):
        assert estimate_cost("muapi/suno-create-music") == 0.10

    def test_unknown_model_zero(self):
        assert estimate_cost("muapi/unknown", n=5) == 0.0

    def test_params_dict_form(self):
        assert estimate_cost("muapi/nano-banana", {"n": 2}) == round(0.039 * 2, 6)

    def test_params_and_overrides_merge(self):
        # kwargs override params
        assert estimate_cost("muapi/nano-banana", {"n": 2}, n=4) == round(0.039 * 4, 6)

    def test_resolution_specific_rate(self):
        register_media_rate("test/res-video", {"per_second_by_resolution": {"720p": 0.07, "1080p": 0.12}})
        assert estimate_cost("test/res-video", duration_seconds=4, resolution="1080p") == round(0.12 * 4, 6)
...
class TestRegisterMediaRate:
    def test_runtime_registration_and_precedence(self):
        ...
        try:
            ...
            assert estimate_cost("muapi/nano-banana", n=2) == round(1.23 * 2, 6)
        finally:
...
def test_top_level_export():
    ...
    assert prompture.estimate_cost("muapi/nano-banana", n=1) == 0.039
