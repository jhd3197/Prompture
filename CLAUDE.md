...
- **`prompture/gateway/`** — framework-agnostic wire formats. `anthropic_format.py` (Anthropic Messages: `anthropic_to_driver`, `stream_anthropic_events`, `live_events_for`) and `responses_format.py` (OpenAI Responses) sit beside the OpenAI chat format; `reasoning_cache.py` replays `reasoning_content` for stateless clients. OpenAI chat format: request→driver messages/options (`to_driver_messages`, `driver_options` incl. `response_format`→`json_mode`), `run_chat`/`arun_chat`, `chat_completion`/`chat_chunk`, `stream_chat_chunks` (with `on_complete` metering hook), `sse`. Used by `prompture serve` and prompture-hub — keep response shaping here, not in servers.
- **Virtual models** — `combo/<name>`, `auto/<mode>`, `fusion/<name>` (`groups/fusion.py`) and aliases resolve in `resilience/virtual.py`, called at the top of `get_driver_for_model` / `get_async_driver_for_model`.
- **`prompture/infra/rate_limits.py`** — `LimitSnapshot` from documented rate-limit response headers (OpenAI/Groq `x-ratelimit-*`, Anthropic `anthropic-ratelimit-*`). SDK drivers hook their httpx client (`attach_rate_limit_hook`) and wrap calls in `capture_rate_limits()`; streams read `stream.response`. Surfaces as `meta["rate_limits"]` only when headers were present. `resilience/headroom.py` feeds these into routing: `HeadroomTracker` keeps the latest snapshot per target and `RoutePlan` moves targets below `RetryPolicy.min_headroom` (default 5%) to the back (`meta["route"]["deprioritized"]`).
- **`prompture/infra/accounts.py`** — `AccountSource` registry for balances/spend from documented endpoints (OpenRouter key, DeepSeek, Moonshot, OpenAI/Anthropic admin cost reports). `get_account_snapshots()` only calls configured sources, caches ~60s, and turns failures into `error` snapshots.
- **`prompture/companion/`** — the companion API that desktop companions (Prompture Desk) read, shared with prompture-hub: `LiveBus` (SSE event bus, asyncio + thread subscribers), `summarize_spend` / `provider_limits` / period windows, `COMPANION_API_VERSION`. `prompture companion` runs a stdlib-only local server over the usage ledger (`LedgerSource`) on 127.0.0.1 with its URL + token in `~/.prompture/companion.json`, so no hub is needed. Usage is tagged per project via `tracker.project()` / `PROMPTURE_PROJECT`.
- **`prompture/infra/otel.py`** / **`infra/compression.py`** — GenAI-convention OpenTelemetry spans via `DriverCallbacks`; conservative message compression for agent loops.
